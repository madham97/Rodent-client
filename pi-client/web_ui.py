          ...
          <div style="font-size:0.82rem; overflow-y:auto; max-height:600px">
            ...
            <table class="table table-sm table-borderless" style="font-size:0.82rem">
              <tbody>
                ...
                <tr><td class="text-nowrap pe-2"><code>image_rotation</code></td><td class="text-muted">Software rotation of the visible feed in degrees clockwise (0/90/180/270) to correct how the CSI camera is physically mounted — corrects orientation without mirroring left/right</td></tr>
                <tr><td class="text-nowrap pe-2"><code>image_hflip</code> / <code>image_vflip</code></td><td class="text-muted">Mirror the visible feed left/right or up/down (true/false). A rotation cannot do this — it preserves handedness, a flip reverses it — so a feed that comes off the sensor mirrored needs this instead. Applied <em>after</em> <code>image_rotation</code>, so the axes refer to the upright image</td></tr>
              </tbody>
            </table>
            ...
            <table class="table table-sm table-borderless" style="font-size:0.82rem">
              <tbody>
                ...
                <tr><td class="text-nowrap pe-2"><code>thermal_norm_min_c</code> / <code>thermal_norm_max_c</code></td><td class="text-muted">Fixed °C range the thermal frame's 0-255 alpha byte is stretched from (default 10.0 / 45.0), the same every frame — not each frame's own min/max, which would let the same real temperature map to different pixel values depending on the scene. Widen if real temperatures clip outside this range</td></tr>
                ...
                <tr><td class="text-nowrap pe-2"><code>thermal_rotation</code></td><td class="text-muted">Rotation of the thermal feed in degrees clockwise (0/90/180/270), for a sensor mounted at a different angle to the camera — the thermal counterpart to <code>image_rotation</code>. Applied before the thermal flips. A 90/270 rotation swaps the frame's aspect ratio, and the result is stretched to the visible frame when fused</td></tr>
                <tr><td class="text-nowrap pe-2"><code>thermal_hflip</code> / <code>thermal_vflip</code></td><td class="text-muted">Mirror the thermal frame so it agrees with the (correctly oriented) visible frame on left/right and up/down — set from a real test, not guessed. Correcting each feed separately is what brings the two into agreement; if the visible frame is the mirrored one, use <code>image_hflip</code> instead so the uploaded photo isn't left mirrored</td></tr>
